...
def set_imc_creds(h_url=None, imc_server=None, imc_port=None, imc_user=None, imc_pw=None):
    ...
    if h_url is None:
        imc_protocol = eval(input("What protocol would you like to use to connect to the "
                             "IMC server: \n Press 1 for HTTP: \n Press 2 for HTTPS:"))
        if imc_protocol == "1":
            h_url = 'http://'
        else:
            h_url = 'https://'
        imc_server = eval(input("What is the ip address of the IMC server?"))
        imc_port = eval(input("What is the port number of the IMC server?"))
        imc_user = eval(input("What is the username of the IMC eAPI user?"))
        imc_pw = eval(input('''What is the password of the IMC eAPI user?'''))
